...
class Florence2Attention(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            # FIX: Handle case where past_key_value might contain None from cache handling
            try:
                if past_key_value[0] is not None and past_key_value[1] is not None:
                    key_states = torch.cat([past_key_value[0], key_states], dim=2)
                    value_states = torch.cat([past_key_value[1], value_states], dim=2)
            except (IndexError, TypeError):
                # If past_key_value is malformed, just use current states
                pass
        else:
            # self_attention
            key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
...
class Florence2Decoder(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        ...
        # past_key_values_length - FIX: Handle DynamicCache
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # FIX: Handle DynamicCache for past_key_value access
            if past_key_values is not None:
                try:
...
class Florence2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            # FIX: Handle DynamicCache which may have empty layers
            try:
                # Try to get past_length from cache
                if hasattr(past_key_values, 'get_seq_length'):
                    # DynamicCache has this method
                ...
                # If anything goes wrong, assume no past
                past_length = 0

            # Some generation methods already pass only the last input ID
            if past_length > 0 and decoder_input_ids.shape[1] > past_length:
                remove_prefix_length = past_length
            elif past_length > 0:
                # Default to old behavior: keep only final ID
                remove_prefix_length = decoder_input_ids.shape[1] - 1
            else:
                # No past, don't remove anything
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel, GenerationMixin):
    ...
    _supports_sdpa = True  # FIX: Add missing attribute for transformers compatibility
