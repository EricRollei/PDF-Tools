"""
ComfyUI Node: Enhanced PDF Image and Text Extractor v0.5.0
Description: 
...
class SmartDoublePageDetector:
    def __init__(self, debug_mode: bool = False):
        ...
        # Add the new double-page detector
    ...
    def analyze_images(self, images: List[Image.Image]) -> List[Dict[str, Any]]:
        ...
        for i, image in enumerate(images):
            if self.debug_mode:
                print(f"\nAnalyzing image {i+1}/{len(images)} ({image.size})")
            
            # Method 1: Advanced double-page detection (MAIN METHOD NOW)
            double_page_result = self.double_page_detector.analyze_double_page_characteristics(image)
            
            # Method 2: Border pattern analysis
            border_result = self.border_analyzer.analyze_borders(image)
            
            if self.debug_mode:
                print(f"  Advanced detection: {double_page_result.get('is_double_page', False)} (confidence: {double_page_result.get('confidence', 0):.3f})")
                print(f"  Border analysis: {border_result['page_type']} (confidence: {border_result['confidence']:.3f})")
            
            # Method 3: Basic geometric analysis
            geometric_result = self._geometric_analysis(image)
            if self.debug_mode:
                print(f"  Geometric: {geometric_result['page_type']} (confidence: {geometric_result['confidence']:.3f})")
            
            # Combine results with focus on advanced detection
            final_result = self._combine_analysis_results_fixed(
                border_result,
                geometric_result,
                double_page_result,
                image,
...
class EnhancedPDFExtractorNode:
    ...
    def extract_pdf_content_enhanced(self, pdf_paths, output_directory, extract_images, extract_text, 
                                   ...
                                   image_segmentation_prompt="pictures inside picture OR text", # Default prompt was different in INPUT_TYPES
    ...
    def _create_enhanced_summary_report(self, reports, total_pdfs, save_options, total_joined, total_enhanced):
        ...
        summary_lines = [
            "=== ENHANCED PDF EXTRACTION SUMMARY v0.5.0 ===",
...
class StreamingPDFProcessor:
    ...
    def _streaming_double_page_joining(self, report: ProcessingReport, 
        ...
        for left_img in left_pages:
            ...
            if processor._validate_spread_pair(left_img, right_img):
...
class EnhancedPDFProcessor:
    def __init__(self, output_dir: str, detector: SmartDoublePageDetector, 
        ...
        self.sam2_florence = None
        try:
            from PDF_tools import SAM2FlorenceIntegration
            sam2_script_path = Path(__file__).parent / "sam2_florence_segmentation.py"
            self.sam2_florence = SAM2FlorenceIntegration(sam2_script_path, debug_mode)
            
            if self.sam2_florence.available and debug_mode:
                print("✅ SAM2+Florence integration available")
        except ImportError:
            if debug_mode:
                print("⚠️ SAM2+Florence integration not available")

        # --- Instantiate SegmentationAnalyzer ---
        self.segmentation_analyzer = None
        if SEGMENT_ANYTHING_AVAILABLE:
            self.segmentation_analyzer = SegmentationAnalyzer(
                dino_config_name=self.config.dino_config_name,
                sam_model_name=self.config.sam_model_name,
                debug_mode=self.debug_mode
            )
            if not self.segmentation_analyzer.available:
                if self.debug_mode: print("EnhancedPDFProcessor: SegmentationAnalyzer failed to initialize or models not found.")
                self.segmentation_analyzer = None 
            else:
                # IMPORTANT: Set the config after successful initialization
                self.segmentation_analyzer.set_config(self.config)
                if self.debug_mode: print("EnhancedPDFProcessor: SegmentationAnalyzer initialized successfully.")
        else:
            if self.debug_mode: print("EnhancedPDFProcessor: SAM library not available, SegmentationAnalyzer not initialized.")
    ...
    def _analyze_page_content_enhanced(self, image: Image.Image, page_num: int, image_index: int) -> Dict[str, Any]:
        ...
        try:
            # analyze_images returns a list of results, take the first one.
            combined_page_analysis = self.detector.analyze_images([image])[0] 
            
            # Update analysis_results with the combined outcome
            analysis_results.update(combined_page_analysis) # This will set page_type, confidence, and store sub-results
    ...
    def process_pdf_enhanced(self, pdf_path: str, extract_images: bool = True, 
        ...
        """Enhanced processing with automatic streaming for large PDFs"""
        ...
        should_stream = streaming_processor._should_use_streaming(pdf_path)  # Call method on streaming_processor instance
        ...
        if should_stream and self.config.memory_optimization:
            ...
            # Use regular processing for smaller PDFs
            report = self._create_initial_report(pdf_path)
            
            try:
                if self.use_pymupdf:
                    self._process_with_pymupdf_enhanced(
                ...
                # Enhanced double-page joining with error recovery
                if extract_images and join_double_pages and len(report.extracted_images) > 1:
                    join_result = self.safe_execute(
                        self._enhanced_double_page_joining,
                        report,
                        fallback_result=None,
                        context="double-page joining"
                    )
    ...
    def _process_with_pymupdf_enhanced(self, pdf_path: Path, report: ProcessingReport,
                                     extract_images: bool, extract_text: bool, 
                                     save_options: str, enable_smart_crop: bool, enhancement_strength: float, 
                                     join_double_pages: bool, use_gpu_acceleration: bool):
        """Enhanced PyMuPDF processing with memory management"""
        doc = fitz.open(str(pdf_path))
        report.total_pages = doc.page_count
        ...
        try:
            ...
            for batch_start in range(0, doc.page_count, batch_size):
                ...
                with self._batch_image_processing() as (processed_count, image_refs):
                    batch_images = []
                    ...
                    for page_num in range(batch_start, batch_end):
                        ...
                        # Extract images
                        if extract_images:
                            page_images = self._extract_page_images_for_analysis_safe(
                                page, page_num, report, image_refs
                            )
                            batch_images.extend(page_images)
                    
                    # Analyze batch
                    if batch_images:
                        self._process_image_batch_safe(
                            batch_images, report, save_options, enable_smart_crop, 
                            enhancement_strength, use_gpu_acceleration
                        )
    ...
    def _apply_smart_cropping_enhanced(self, image: Image.Image, analysis: Dict) -> Image.Image:
        """Enhanced smart cropping with multiple fallback methods"""
        
        # Method 1: Try segmentation-based cropping first
        segmentation_result = self._try_segmentation_cropping(image, analysis)
        if segmentation_result["success"]:
            if self.debug_mode:
                print(f"    ✅ Segmentation cropping successful: {segmentation_result['method']}")
            return segmentation_result["image"]
        
        # Method 2: Try CV2-based content detection
        cv2_result = self._try_cv2_content_detection(image)
        if cv2_result["success"]:
            if self.debug_mode:
                print(f"    ✅ CV2 content detection successful")
            return cv2_result["image"]
        
        # Method 3: Try adaptive background removal
        adaptive_result = self._try_adaptive_background_removal(image)
        if adaptive_result["success"]:
            if self.debug_mode:
                print(f"    ✅ Adaptive background removal successful")
            return adaptive_result["image"]
        
        # Method 4: Fallback to original with minimal border cleanup
        cleaned_result = self._minimal_border_cleanup(image)
        if self.debug_mode:
            print(f"    🔄 Using minimal border cleanup fallback")
    ...
    def _enhanced_double_page_joining(self, report: ProcessingReport):
        ...
        for left_img in left_pages:
            ...
            if self._validate_spread_pair(left_img, right_img):
...
# Node registration
NODE_CLASS_MAPPINGS = {
    "Eric_PDF_Extractor_Enhanced_v05": EnhancedPDFExtractorNode
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "Eric_PDF_Extractor_Enhanced_v05": "Eric PDF Extractor Enhanced v0.5.0"
}