...
"""
Enhanced PDF Extractor v0.9.0 - Layer Detection + Analysis Engine
Major upgrade with automatic PDF layer detection for super-fast extraction

Key Features v0.9.0:
- Automatic PDF layer detection (Optional Content Groups)
- Super-fast layer-based extraction when layers detected
...
- Optional fast mode: only extract if layers present
...
try:
    ...
    from PDF_tools.florence2_scripts.image_quality_analyzer import ImageQualityAnalyzer
    MODERN_ENHANCER_AVAILABLE = True
except ImportError:
    try:
        from .florence2_scripts.modern_image_enhancer import ModernImageEnhancer
        from .florence2_scripts.image_quality_analyzer import ImageQualityAnalyzer
        MODERN_ENHANCER_AVAILABLE = True
    except ImportError:
...
class EnhancedPDFExtractorNode:
    """
    Enhanced PDF Extractor v09 with automatic layer detection
    - Detects PDF layers automatically for super-fast extraction
    - Integrated analysis engine (Surya Layout + Florence2)
    - Modern image enhancement
    """
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "prefer_native_images": ("BOOLEAN", {"default": True, "tooltip": "When enabled, if native/embedded images are found, skip AI detection entirely (faster, cleaner). Disable only for flattened/poor quality PDFs."}),
                # layers_only_mode removed - node now always prefers layers automatically (check layers_extracted output)
            },
            "optional": {
                ...
                # === ENHANCEMENT MODE (Main Control) ===
                "enhancement_mode": ([
                    "auto", "none", "manual", "preset_minimal", "preset_magazine", "preset_scanned", "preset_vintage"
                ], {"default": "auto", "tooltip": "auto=Analyzes each image automatically (best for varied quality), none=No enhancement (fastest ~5s), manual=Use sliders below, preset_minimal=Light touch for high-quality, preset_magazine=Standard digital, preset_scanned=Scanned docs/photos, preset_vintage=Old/compressed sources"}),
                
                # === MANUAL CONTROLS (used when mode=manual) ===
                "denoise_strength": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.1, "tooltip": "Noise reduction: 0.0=none, 0.3=light, 0.5=medium, 0.8=heavy. Auto mode detects optimal value."}),
                "sharpen_strength": ("FLOAT", {"default": 0.5, "min": 0.0, "max": 1.0, "step": 0.1, "tooltip": "Sharpening: 0.0=none, 0.5=moderate, 1.0=maximum. Auto mode detects optimal value."}),
                "tone_map_strength": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.1, "tooltip": "Tone mapping/contrast: 0.0=none, 0.5=moderate, 1.0=maximum. Auto mode detects optimal value."}),
                "color_enhance_strength": ("FLOAT", {"default": 0.3, "min": 0.0, "max": 1.0, "step": 0.1, "tooltip": "Color vibrancy: 0.0=none, 0.5=moderate, 1.0=maximum. Auto mode detects optimal value."}),
                "artifact_removal": ("BOOLEAN", {"default": False, "tooltip": "Remove JPEG compression artifacts. Auto mode enables if compression detected."}),
                
                # === SHARPENING METHOD ===
                "sharpening_method": ([
                    "gpu_basic", "smart_adaptive", "hiraloam_gentle", "edge_directional", "multiscale_quality", "guided_filter"
                ], {"default": "gpu_basic", "tooltip": "gpu_basic=Fast GPU sharpening (recommended), smart_adaptive=CPU adaptive, hiraloam_gentle=natural subtle, edge_directional=edge-aware, multiscale_quality=highest quality (slow), guided_filter=edge-preserving"}),
    ...
    RETURN_TYPES = ("IMAGE", "IMAGE", "LIST", "STRING", "DICT", "STRING", "BOOLEAN")  # extracted_images, enhanced_images now IMAGE
    RETURN_NAMES = ("extracted_images", "enhanced_images", "page_analysis", "extracted_text", "extraction_stats", "output_path", "has_layers")
    OUTPUT_IS_LIST = (True, True, False, False, False, False, False)  # images are lists of individual tensors
    ...
    def __init__(self):
        ...
        print("🚀 Enhanced PDF Extractor v09 - Layer Detection + Analysis Engine")
    ...
    def _get_preset_values(self, preset_name: str) -> Dict[str, any]:
        """Get enhancement parameter values for a preset mode"""
        presets = {
            "preset_minimal": {
                "denoise": 0.0,
                "sharpen": 0.2,
        ...
        return presets.get(preset_name, presets["preset_minimal"])

    def _resolve_enhancement_params(self, mode: str, image: Image.Image, 
                                   denoise: float, sharpen: float, tone_map: float, 
                                   color: float, artifacts: bool, debug: bool = False) -> Dict[str, any]:
        """
        Resolve enhancement parameters based on mode
        
        Args:
            mode: Enhancement mode (auto, none, manual, preset_*)
            image: PIL Image to analyze (for auto mode)
            denoise, sharpen, tone_map, color, artifacts: Manual parameter values
            debug: Print analysis results
            
        Returns:
            Dict with resolved parameters: denoise, sharpen, tone_map, color_pop, artifact_removal
        """
        
        if mode == "none":
            # No enhancement - return all zeros
            return {
                "denoise": 0.0,
                "sharpen": 0.0,
                "tone_map": 0.0,
            ...
            if not hasattr(self, 'quality_analyzer'):
                self.quality_analyzer = ImageQualityAnalyzer(debug_mode=debug)
            
            analysis = self.quality_analyzer.analyze(image)
            
            return {
                "denoise": analysis["denoise"],
                "sharpen": analysis["sharpen"],
                "tone_map": analysis["tone_map"],
                "color_pop": analysis["color_enhance"],
                "artifact_removal": analysis["artifact_removal"]
            }
        
        elif mode.startswith("preset_"):
            # Use preset values
            return self._get_preset_values(mode)
        
        else:  # mode == "manual"
            # Use user-specified values directly
            return {
                "denoise": denoise,
                "sharpen": sharpen,
                "tone_map": tone_map,
                "color_pop": color,
                "artifact_removal": artifacts
            }
    ...
    def extract_enhanced(self, pdf_path: str, output_directory: str = "output/pdf_extraction",
                        ...
                        join_spreads: bool = True, extract_text: bool = True, prefer_native_images: bool = True,
                        ...
                        # New clear enhancement parameters
                        enhancement_mode: str = "auto",
                        denoise_strength: float = 0.0,
                        sharpen_strength: float = 0.5,
                        tone_map_strength: float = 0.0,
                        color_enhance_strength: float = 0.3,
                        artifact_removal: bool = False,
                        sharpening_method: str = "gpu_basic",
                        # Other parameters
                        ...
                        debug_mode: bool = True) -> Tuple[List, List, List, str, Dict, str, bool]:
        """Enhanced PDF extraction with automatic layer detection - supports single file or folder"""
        
        self.debug_mode = debug_mode
        
        # Fix: Handle legacy workflow values for spread_detection_strategy
        # Old workflows may have boolean False instead of "disabled"
        valid_strategies = ["semantic_layout", "florence2_boxes", "hybrid", "disabled"]
        if spread_detection_strategy not in valid_strategies:
            if str(spread_detection_strategy).lower() in ['false', '0', 'none']:
                spread_detection_strategy = "disabled"
                if debug_mode:
                    print(f"⚠️ Invalid spread_detection_strategy value, defaulting to 'disabled'")
            else:
                spread_detection_strategy = "hybrid"
                if debug_mode:
                    print(f"⚠️ Unknown spread_detection_strategy '{spread_detection_strategy}', defaulting to 'hybrid'")
        
        # Validate sharpening_method
        valid_sharpening = ["auto", "smart", "hiraloam", "edge_directional", "multiscale", "multiscale_quality", "guided", "basic", "gpu_basic", "smart_adaptive", "hiraloam_gentle", "guided_filter"]
        if sharpening_method not in valid_sharpening:
            if debug_mode:
                print(f"⚠️ Unknown sharpening_method '{sharpening_method}', defaulting to 'auto'")
        ...
        if os.path.exists(pdf_path) and os.path.isdir(pdf_path):
            return self._extract_batch_v09(
                pdf_path, output_directory, remove_text_layers, enable_image_enhancement,
                join_spreads, extract_text, prefer_native_images, start_page, end_page,
                min_image_size, dpi, surya_confidence, florence2_confidence,
                enhancement_mode, denoise_strength, sharpen_strength, tone_map_strength,
                color_enhance_strength, artifact_removal, sharpening_method,
                spread_detection_strategy, avoid_full_page_boxes, smart_crop_captions,
                save_images_to_disk, save_text_to_file, debug_mode
            )
        
        # Single file processing (original logic)
        return self._extract_single_pdf_v09(
            pdf_path, output_directory, remove_text_layers, enable_image_enhancement,
            join_spreads, extract_text, prefer_native_images, start_page, end_page,
            min_image_size, dpi, surya_confidence, florence2_confidence,
            enhancement_mode, denoise_strength, sharpen_strength, tone_map_strength,
    ...
    def _extract_single_pdf_v09(self, pdf_path: str, output_directory: str,
                               remove_text_layers: bool, enable_image_enhancement: bool,
                               join_spreads: bool, extract_text: bool, prefer_native_images: bool,
                               start_page: int, end_page: int, min_image_size: int, dpi: int,
                               surya_confidence: float, florence2_confidence: float,
                               # New enhancement parameters
                               enhancement_mode: str, denoise_strength: float, sharpen_strength: float,
                               tone_map_strength: float, color_enhance_strength: float, artifact_removal: bool,
                               sharpening_method: str,
                               spread_detection_strategy: str, avoid_full_page_boxes: bool,
                               smart_crop_captions: bool, save_images_to_disk: bool,
                               save_text_to_file: bool, debug_mode: bool) -> Tuple[List, List, List, str, Dict, str, bool]:
        ...
        # STEP 1: Detect PDF layers (always check - it's fast!)
        try:
            with fitz.open(pdf_path) as doc:
                layer_info = self._detect_layers(doc, debug_mode)
                has_layers = layer_info["has_layers"]
                
                if has_layers:
                    if debug_mode:
                        print(f"\n✨ PDF Layer Detection:")
                        print(f"   📋 Found {layer_info['layer_count']} layers")
                        for layer in layer_info['layers']:
                            print(f"      • '{layer['name']}' ({'ON' if layer['visible'] else 'OFF'})")
                        print(f"   🚀 Using optimized layer-aware extraction")
                else:
                    if debug_mode:
                        print(f"\n📄 No OCG layers detected - using standard extraction")
                        print(f"   ℹ️  Note: This PDF may have images but not OCG (Optional Content Group) layers")
                    # Note: Always proceeds with extraction - layers are preferred but not required
        except Exception as e:
            if debug_mode:
        ...
        # Extract pages as images (now layer-aware)
        ...
        for page in analyzed_pages:
            ...
            page_images, page_enhanced, page_saved_paths = self._process_page_images(
                ...
                avoid_full_page_boxes, crop_recommendations, prefer_native_images,
                enable_image_enhancement, 
                enhancement_mode, denoise_strength, sharpen_strength, tone_map_strength,
                color_enhance_strength, artifact_removal, sharpening_method,
                output_path, save_images_to_disk, debug_mode
            )
        ...
        # Add layer info to extraction stats
        if has_layers and layer_info:
            extraction_stats["layer_detection"] = layer_info
            
            # Save layer info to file
            layer_file = Path(output_path) / "layer_info.json"
            with open(layer_file, 'w', encoding='utf-8') as f:
                json.dump(layer_info, f, indent=2)
            if debug_mode:
                print(f"   📋 Layer info saved: {layer_file}")

        # When OUTPUT_IS_LIST=True, ComfyUI requires at least one item in the list
        # If no images were extracted, provide a dummy 1x1 tensor as placeholder
        if len(extracted_images) == 0:
            dummy_tensor = torch.zeros((1, 1, 1, 3), dtype=torch.float32)
            extracted_images = [dummy_tensor]
            enhanced_images = [dummy_tensor]
            if debug_mode:
                print(f"   ⚠️  No images extracted - returning placeholder tensor")

        return (extracted_images, enhanced_images, page_analysis, combined_text, extraction_stats, output_path, has_layers)
    
    def _extract_batch_v09(self, folder_path: str, output_directory: str,
                          remove_text_layers: bool, enable_image_enhancement: bool,
                          join_spreads: bool, extract_text: bool,
                          start_page: int, end_page: int, min_image_size: int, dpi: int,
                          surya_confidence: float, florence2_confidence: float,
                          # New enhancement parameters
                          enhancement_mode: str, denoise_strength: float, sharpen_strength: float,
        ...
        if not pdf_files:
            ...
            print(msg)
            # Provide placeholder tensor for OUTPUT_IS_LIST
            dummy_tensor = torch.zeros((1, 1, 1, 3), dtype=torch.float32)
            return ([dummy_tensor], [dummy_tensor], [], msg, {"error": "no_pdfs"}, output_directory, False)
        
        if debug_mode:
            print(f"📚 Found {len(pdf_files)} PDF files")
            print(f"🏃 Mode: Always prefers layers when available (check layers_extracted output)")
            print(f"{'='*60}\n")
        ...
        for idx, pdf_file in enumerate(pdf_files, 1):
            ...
            try:
                ...
                extracted, enhanced, analysis, text, stats, output_path, has_layers = self._extract_single_pdf_v09(
                    pdf_file, output_directory, remove_text_layers, enable_image_enhancement,
                    join_spreads, extract_text, start_page, end_page,
                    min_image_size, dpi, surya_confidence, florence2_confidence,
                    enhancement_mode, denoise_strength, sharpen_strength, tone_map_strength,
                    color_enhance_strength, artifact_removal, sharpening_method, spread_detection_strategy,
                    avoid_full_page_boxes, smart_crop_captions, save_images_to_disk,
                    save_text_to_file, debug_mode
                )
                
                pdf_elapsed = time.time() - pdf_start
                
                # Track stats
                img_count = len(extracted) if extracted else 0
                enh_count = len(enhanced) if enhanced else 0
                
                if img_count > 0:
                    batch_stats["processed"] += 1
                    batch_stats["total_images"] += img_count
                    batch_stats["total_enhanced"] += enh_count
                    if has_layers:
                ...
                # Store result
                batch_stats["processing_times"].append(pdf_elapsed)
                batch_stats["results"].append({
                    ...
                    "time": pdf_elapsed,
                    "status": "processed" if img_count > 0 else "skipped"
                })
        ...
        batch_summary_file = output_dir / f"batch_summary_v09_{time.strftime('%Y%m%d_%H%M%S')}.json"
        ...
        summary = (
            f"✅ Batch Extraction Complete (v09)\n"
            ...
            f"✨ Enhanced: {batch_stats['total_enhanced']}\n"
        ...
        if len(all_extracted) == 0:
            dummy_tensor = torch.zeros((1, 1, 1, 3), dtype=torch.float32)
            all_extracted = [dummy_tensor]
            all_enhanced = [dummy_tensor]
            if debug_mode:
                print(f"   ⚠️  No images extracted in batch - returning placeholder tensor")
        
        return (all_extracted, all_enhanced, all_analysis, combined_text, batch_stats, str(output_dir), has_any_layers)
        
    def _detect_layers(self, doc: fitz.Document, debug_mode: bool = False) -> Dict:
        ...
        try:
            # Method 1: Try get_layers() API
            layers = doc.get_layers()
            
            if debug_mode:
                print(f"   🔍 Layer detection method 1 (get_layers): {layers}")
            ...
            # Method 2: Check for OCG dictionary in PDF catalog (more robust)
            if not layer_info["has_layers"]:
                try:
                    # Access the PDF catalog
                    catalog = doc.pdf_catalog()
                    if debug_mode:
                        print(f"   🔍 Checking PDF catalog for OCProperties...")
                    
                    # Check if OCProperties exists
                    if catalog and "OCProperties" in catalog:
                        if debug_mode:
                            print(f"   ✅ Found OCProperties in catalog!")
                        
                        ocprops = catalog["OCProperties"]
                        
                        # Get OCGs (Optional Content Groups)
                        if "OCGs" in ocprops:
                            ocgs = ocprops["OCGs"]
                            if debug_mode:
                                print(f"   📦 Found {len(ocgs) if isinstance(ocgs, list) else 1} OCG(s)")
                            
                            layer_info["has_layers"] = True
                            
                            # Extract layer information from OCGs
                            if isinstance(ocgs, list):
                                layer_info["layer_count"] = len(ocgs)
                                for i, ocg in enumerate(ocgs):
                                    layer_name = ocg.get("Name", f"Layer {i+1}") if isinstance(ocg, dict) else f"Layer {i+1}"
                                    layer_info["layers"].append({
                                        "name": str(layer_name),
                                        "number": i,
                                ...
                                layer_info["layer_count"] = 1
                                layer_name = ocgs.get("Name", "Layer 1") if isinstance(ocgs, dict) else "Layer 1"
                                layer_info["layers"].append({
                                    "name": str(layer_name),
                                    "number": 0,
                                    "visible": True,
                        ...
                        if debug_mode:
                            print(f"   ❌ No OCProperties found in catalog")
                except Exception as e2:
                    if debug_mode:
                        print(f"   ⚠️  Catalog check failed: {e2}")
        
        except Exception as e:
            # If layer detection fails, just continue without layers
            if debug_mode:
                print(f"   ⚠️  Layer detection error: {e}")
        ...
        for i, detection in enumerate(final_detections):
            try:
                ...
                if source == "native_embedded":
                    ...
                    # Skip enhancement if profile is "None" or if auto-detected as unnecessary
                    if enhancement_profile == "None" or enhancement_rec == "none":
    ...
    def _create_joined_spread_page(self, left_page: PDFPage, right_page: PDFPage, 
        ...
        # Combine native images from both pages, adjusting right page coordinates
        combined_native_images = []
        
        # Add left page native images (no coordinate adjustment needed)
        if left_page.native_images:
            combined_native_images.extend(left_page.native_images)
        
        # Add right page native images (adjust coordinates by left page width)
        if right_page.native_images:
            for native_img in right_page.native_images:
                adjusted_img = native_img.copy()
                # Adjust bbox coordinates
                if "bbox" in adjusted_img:
                    bbox = adjusted_img["bbox"]
                    adjusted_img["bbox"] = [
                        bbox[0] + left_img.width,
                        bbox[1],
                        bbox[2] + left_img.width,
                        bbox[3]
                    ]
                combined_native_images.append(adjusted_img)
        
        if debug_mode:
            left_count = len(left_page.native_images) if left_page.native_images else 0
            right_count = len(right_page.native_images) if right_page.native_images else 0
            print(f"   📊 Combined analysis: {len(combined_native_images)} native images ({left_count} from left, {right_count} from right)")
        ...
        joined_page = PDFPage(
            ...
            native_images=combined_native_images  # FIX: Preserve native images!
        )
    ...
    def _process_page_images(self, page: PDFPage, min_image_area: int, 
                            florence2_confidence: float, avoid_full_page_boxes: bool,
                            crop_recommendations: List[Dict], prefer_native_images: bool, enable_enhancement: bool,
                            # New enhancement parameters
                            enhancement_mode: str, denoise_strength: float, sharpen_strength: float,
                            tone_map_strength: float, color_enhance_strength: float, artifact_removal: bool,
                            sharpening_method: str,
        ...
        native_images_added = 0
        for native_img in native_images:
            area = native_img.get("area", 0)
            size = native_img.get("size", (0, 0))
            min_dimension = min(size[0], size[1])
            min_size_px = int(min_image_area ** 0.5)  # Convert area back to side length
            
            # Check if the MINIMUM dimension meets the threshold (not just total area)
            if min_dimension >= min_size_px:
                ...
                native_images_added += 1
                
                if debug_mode:
                    print(f"       ✅ Added native image: {native_img['size']}, format: {native_img['format']}, min_dim: {min_dimension}px")
            else:
                if debug_mode:
                    print(f"       ❌ Skipped native image: {native_img['size']} (min_dim {min_dimension}px < {min_size_px}px threshold)")
        
        if debug_mode and native_images:
            min_size_px = int(min_image_area ** 0.5)
            print(f"       📊 Native images: {len(native_images)} total, {native_images_added} added (min_dimension >= {min_size_px}px)")

        # STEP 1: Check if we should skip AI detection entirely
        # If prefer_native_images=True AND we have valid native images, SKIP to final processing
        skip_ai_detection = prefer_native_images and native_images_added > 0
        
        if skip_ai_detection:
            if debug_mode:
                print(f"       ✅ Prefer native images enabled: Skipping all AI detection (using {native_images_added} native images only)")
            # Jump directly to STEP 4 (final processing) - skip Florence2 and Surya
            
        # STEP 1A: Florence2 detections (only if NOT skipping AI)
        if not skip_ai_detection:
            florence2_detections = page.analysis_results.get("florence2_rectangles", [])
            
            if debug_mode:
                print(f"       📦 Florence2 detections found: {len(florence2_detections)}")
            
            # Add all valid Florence2 detections (filter out overlaps with native images later)
            for f2_detection in florence2_detections:
                ...
                if (confidence >= florence2_confidence and 
                    ...
                    # Check overlap with ALL existing detections (avoid duplicates)
                    overlaps_with_existing = False
                    max_overlap = 0.0
                    overlap_source = None
                    
                    for existing_detection in final_detections:
                        existing_bbox = existing_detection.get("bbox", [0, 0, 0, 0])
                        existing_source = existing_detection.get("detection_source", "unknown")
                        overlap_ratio = self._calculate_overlap_ratio(bbox, existing_bbox)
                        
                        # 70% overlap for native images, 80% for Florence2 duplicates
                        overlap_threshold = 0.7 if existing_source == "native_embedded" else 0.8
                        
                        if overlap_ratio > overlap_threshold:
                            overlaps_with_existing = True
                            max_overlap = overlap_ratio
                            overlap_source = existing_source
                            if debug_mode:
                                print(f"       🚫 Skipped F2 detection (overlap={overlap_ratio:.2f} with {overlap_source}): {bbox}")
                            break
                    
                    # ADDITIONAL CHECK: Is this F2 box already covered by multiple native images?
                    # (e.g., landscape page with two side-by-side images, F2 detects one big box)
                    if not overlaps_with_existing:
                        native_detections = [d for d in final_detections if d.get("detection_source") == "native_embedded"]
                        if len(native_detections) >= 2:
                            total_native_coverage = self._calculate_multi_box_coverage(bbox, 
                                [d.get("bbox", [0, 0, 0, 0]) for d in native_detections])
                            
                            if total_native_coverage > 0.6:  # 60% of F2 box is covered by native images
                                overlaps_with_existing = True
                                overlap_source = "multiple_native_images"
                                if debug_mode:
                                    print(f"       🚫 Skipped F2 detection (covered {total_native_coverage:.1%} by {len(native_detections)} native images): {bbox}")
                    
                    if overlaps_with_existing:
                        continue  # Skip this Florence2 detection
        ...
        for detection in final_detections:
            ...
            # Skip size filter for native images - they were already validated when extracted
            if source == "native_embedded":
                filtered_detections.append(detection)
                if debug_mode:
                    native_img = detection.get("native_image")
                    if native_img:
                        print(f"       ✅ Keeping native image: {native_img.size} (already validated)")
                continue
            
            # For AI-detected images, check bbox size
            bbox = detection.get("bbox", [0, 0, 0, 0])
            if len(bbox) >= 4:
                width = bbox[2] - bbox[0]
        ...
        for i, detection in enumerate(filtered_detections):
            try:
                ...
                # Apply SMART enhancement using new parameter system
                enhanced_image = cropped
                if enable_enhancement and self.image_enhancer:
                    try:
                        # Resolve enhancement parameters based on mode
                        params = self._resolve_enhancement_params(
                            enhancement_mode, cropped,
                            denoise_strength, sharpen_strength, tone_map_strength,
                            color_enhance_strength, artifact_removal, debug_mode
                        )
                        
                        # Check if enhancement should be skipped
                        if (params["denoise"] == 0.0 and params["sharpen"] == 0.0 and 
                            params["tone_map"] == 0.0 and params["color_pop"] == 0.0):
                            # No enhancement needed
                            enhanced_image = cropped
                            if debug_mode:
                                print(f"           ⏭️ Enhancement skipped (mode: {enhancement_mode})")
                        else:
                            # Apply enhancement with resolved parameters
                            enhanced_image = self.image_enhancer.enhance_image_with_params(
                                cropped,
                                denoise=params["denoise"],
                                sharpen=params["sharpen"],
                                tone_map=params["tone_map"],
                                color_pop=params["color_pop"],
                                artifact_removal=params["artifact_removal"],
                                subject_boxes=None,
                                sharpening_method=sharpening_method
                            )
                            if debug_mode:
                                if enhancement_mode == "auto":
                                    print(f"           ✨ Auto-enhanced: D:{params['denoise']:.2f} S:{params['sharpen']:.2f} T:{params['tone_map']:.2f} C:{params['color_pop']:.2f}")
                                elif enhancement_mode.startswith("preset_"):
                                    preset_name = enhancement_mode.replace("preset_", "").title()
    ...
    def _calculate_multi_box_coverage(self, target_bbox: List[int], source_bboxes: List[List[int]]) -> float:
        """
        Calculate what percentage of target_bbox is covered by the union of multiple source_bboxes.
        Used to detect when a Florence2 detection spans multiple native images.
        
        Example: F2 detects [0, 0, 1000, 500], native images are [0, 0, 450, 500] and [550, 0, 1000, 500]
        Combined they cover ~90% of the F2 box (with 100px gap in middle)
        """
        ...
        # Calculate total area covered by source boxes (accounting for overlaps)
        covered_area = 0.0
        for source_bbox in source_bboxes:
            sx1, sy1, sx2, sy2 = source_bbox
            
            # Calculate intersection with target
            inter_x1 = max(x1, sx1)
            inter_y1 = max(y1, sy1)
            inter_x2 = min(x2, sx2)
            inter_y2 = min(y2, sy2)
            ...
            if inter_x2 > inter_x1 and inter_y2 > inter_y1:
                covered_area += (inter_x2 - inter_x1) * (inter_y2 - inter_y1)
        
        # Note: This is a simplified calculation that doesn't account for overlaps
        # between source boxes, but for native images that's usually fine
        coverage_ratio = min(1.0, covered_area / target_area)
...
# Node registration
NODE_CLASS_MAPPINGS = {
    "EnhancedPDFExtractor_v09": EnhancedPDFExtractorNode,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "EnhancedPDFExtractor_v09": "Enhanced PDF Extractor v09 (Layer Detection)",
}